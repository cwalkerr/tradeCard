...
module.exports = () => {
   ...
   * DEFINE ASSOCIATIONS
   */
  // series has many sets / set belongs to one series
  ...
  // retreat cost belongs to card & energy type
  ...
  const getCard = async (cardId) => {
    ...
    if (card) {
      return card.get({ plain: true }); // cant remember if this is needed anymore
    } else {
      return null;
    }
  };
   ...
   */
  const getCardTile = async ({ limit, offset }) => {
